...
from models import MPNN_LSTM, LSTM, MPNN, prophet, arima, TemporalMGN
...
def mgn_train(epoch, adj, features, y, graph_seq_embed, batch_time):
    adj = adj.to_dense()
    edge_index, edge_attr = dense_to_sparse(adj)
    # print(batch_time)
    # print(graph_seq_embed.size())
    # print(edge_index.size())
    # print(features.size())
    # print(edge_attr.size())
    output, latent = model(batch_time, graph_seq_embed, edge_index, features, edge_attr[:, None])
    loss_train = F.mse_loss(output, y)
    return output, loss_train, latent
...
def mgn_test(adj, features, y, graph_seq_embed, batch_time):
    adj = adj.to_dense()
    edge_index, edge_attr = dense_to_sparse(adj)
    output, latent = model(batch_time, graph_seq_embed, edge_index, features, edge_attr[:, None])
    loss_test = F.mse_loss(output, y)
    return output, loss_test, latent
            ...
            for shift in list(range(0,args.ahead)):
                ...
                for test_sample in range(args.start_exp,n_samples-shift):#
                    ...
                    while(not stop):#
                        ...
                        elif(args.model=="T_MGN"):

                            model = TemporalMGN(clusters=[32], num_layers=5, node_dim=nfeat, edge_dim=1, hidden_dim=64, z_dim=64, num_classes=y_train[0].size()[0], time_seq_length=69).to(device)
                        ...
                        for epoch in range(args.epochs):    
                            ...
                            if(args.model=="T_MGN"):
                                ...
                                for batch in range(n_train_batches):
                                    # print(adj_train[batch].size())
                                    # print(features_train[batch].size())
                                    # print(y_train[batch].size())
                                    if batch == 0:
                                        graph_seq_embed = torch.empty((0, 0)).to(device)
                                    else: 
                                        graph_seq_embed = torch.cat(graph_seq_embed_list, dim=0).to(device)
                                    output, loss, cur_graph_seq_embed = mgn_train(epoch, adj_train[batch], features_train[batch], y_train[batch], graph_seq_embed, batch)
                                    graph_seq_embed_list.append(cur_graph_seq_embed)
                                    loss_ep += loss
                                    train_loss.update(loss.data.item(), output.size(0))
                                loss_ep = loss_ep/n_train_batches
                                loss_ep.backward()
                                optimizer.step()
                                optimizer.zero_grad()
                            else:
                            ...
                            if(args.model=="T_MGN"):
                                graph_seq_embed = torch.empty((0, 0)).to(device)
                                output, val_loss, latent = mgn_test(adj_val[0], features_val[0], y_val[0], graph_seq_embed, 0)
                            else:
